...
Among them, \code{core} is mainly implemented by \code{C++}, which has about \ascii{26} million lines of code.
...
\ascii{Python} defines and implements the programming model of \tf{}, and opens \ascii{API} for programmers. The source structure is as follows, which is also the focus of this book.
...
Among them, the component is implemented by \code{Python}, which has about \ascii{18} million lines of code.
...
Since the \tf{} community is quite active, the changes to \code{contrib} are quite frequent. As of \ascii{1.4}, there are about \ascii{23} million lines of code, mainly designed and implemented by \ascii{Python}. The interface, partially run by \ascii{C++}.
...
Among them, \ascii{StreamExecutor} is implemented using \code{C++}, which has about \ascii{2.5} million lines of code.
...
\ascii{XLA} technology is currently in the initial stage of research and development, and is a more active research direction in the community. As of now, the code size is approximately \ascii{12.5} million lines, mainly using \ascii{C++}.
...
When the system installs \ascii{CUDA} to calculate the \ascii{GPU} card with compatibility greater than or equal to \ascii{3.0}, you need to install the \ascii{CUDA} toolkit and its \ascii{cuDNN} to implement\ The tf{} runtime's \ascii{GPU} acceleration. It is recommended to download \ascii{CUDA Toolkit 8} and above from the \ascii{NVIDIA} website and install it into the system to configure related environment variables.
